...
echo "List of chromosomes to skip (space-separated, e.g., "1 2 3")"
echo "enter them if you wanna skip"

read skip_chromosomes

# Iterate over chromosomes 1 to 22 and X
for chr in $(seq 1 22) X; do
    # Check if the current chromosome is in the skip list
    if [[ " $skip_chromosomes " =~ " $chr " ]]; then
        echo "Chromosome ${chr} is in the skip list. Skipping."
        continue
    fi
    ...
    if ls ukb23374_c${chr}_b*.vcf.gz 1> /dev/null 2>&1; then
        ...
        if [[ "$answer" == "yes" ]]; then 
            ...
            command="bcftools concat --naive -f chr${chr}.merge.list --threads 10 -Oz -o ${out_name} && tabix ${out_name}"
