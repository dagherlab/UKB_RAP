...
for target_chromosome in "${target_chromosomes[@]}"; do
    ...
    if [ "$line_count" -gt 0 ]; then
        ...
        batch_number=$(dx ls "project-GYpQ7KQJb3qV67g26PBqjgf6:/Bulk/Exome sequences/Population level exome OQFE variants, pVCF format - final release/ukb23157_c${target_chromosome}_b*_v1.vcf.gz"|wc -l)
        # iterate each batch
        ## dx upload $output_file --path temp/
        for i in $(seq 0 $batch_number);do
            # you may wanna configure this command for a smaller instance
            dx run app-swiss-army-knife --instance-type mem1_ssd1_v2_x8 -y -iin="project-GYpQ7KQJb3qV67g26PBqjgf6:/Bulk/Exome sequences/Population level exome OQFE variants, pVCF format - final release/ukb23157_c${target_chromosome}_b${i}_v1.vcf.gz" -iin="project-GYpQ7KQJb3qV67g26PBqjgf6:/Bulk/Exome sequences/Population level exome OQFE variants, pVCF format - final release/ukb23157_c${target_chromosome}_b${i}_v1.vcf.gz.tbi" -iin="project-GYpQ7KQJb3qV67g26PBqjgf6:temp/${bed_file}" -icmd="bcftools view -Oz -o UKB_RAP_chr${target_chromosome}_b${i}_${bed_file}.vcf.gz -R ${bed_file} ukb23157_c${target_chromosome}_b${i}_v1.vcf.gz" --destination ${out} --brief --priority low
        done 
